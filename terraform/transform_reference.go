...
// instance, and attaches the addresses to the node itself to be recorded in
// the state. These dependencies are only used for instance destruction without
// configuration. This always relies on the ReferenceTransformer being
// executed, but isn't included in the ReferenceTransformer because it must
// only be run on the complete graph
